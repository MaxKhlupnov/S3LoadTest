...
public class Upload extends AbstractOperation {
    ...
    public OperationResult call(){
        ...
        try {
            ...
            LOG.error("MD5 hash algorithm is nou supported");
            return null;
        }
        for (int i = 0; i < n; i++) {
            final byte data[] = RandomDataGenerator.generate(size);
            final String key = String.format("%06d",i);//UUID.randomUUID().toString();
            ...
            //calc hash
            byte[] digest = md5Hash.digest(data);
            BigInteger bigInt = new BigInteger(1,digest);
            String hashtext = bigInt.toString(16);

            objectMetadata.addUserMetadata("md5",hashtext);
